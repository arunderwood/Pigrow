    ...
    if setting in used_gpio_num:
        ...
        if int(setting) in valid_gpio:
            ...
            if direction.lower() in ["0", "l", "low", "down", "off"]:
                ...
                pi_set[device] = setting
                save_settings()
                print("Set " + device + " to " + setting +
                      " with it's normal state as " + direction)
            else:
...
def show_reddit_menu():
    ...
    if option == "1":
        ...
        if i_my_client_secret == '':
            print("Leaving Client Secret set to; " + my_client_secret +
        ...
        try:
            ...
            whereto.message('Test message from setup.py',
